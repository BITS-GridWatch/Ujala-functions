...
setInterval(async () => {
    ...
    if ((new Date() - new Date(timediff)) > FIFTEEN_MINUTES) {
        ...
        await db.ref("Cutoff/CCAC")
            .child(timediff).child("server").child(key)
            .set(snap.val());
            ...
            .then(async snap => {
                ...
                snap.forEach(childSnap => {
                    ...
                    if (uuid2 !== childSnap.key) {
                        //remove nodes of CCAC only if time difference is >= 15 mins
                        if (uuid1 === "CCAC" && Math.abs(
                            new Date(currTime) - new Date(childSnap.val().Time) >=
                            THIRTY_MINUTES
                        )) {
                            console.log("CCAC data removed: " + childSnap.key);
                            childSnap.ref.remove();
                        }
                        //Send nodes to Data only if time difference is >= 30 mins
                        else if (
                            Math.abs(
                                new Date(currTime) - new Date(childSnap.val().Time) >=
                                THIRTY_MINUTES
